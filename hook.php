...
function plugin_uninstall_MassiveActions($type)
{
    ...
    if (in_array($type, $UNINSTALL_TYPES)) {
        return ["PluginUninstallUninstall:uninstall" => "<i class='" . PluginUninstallReplace::getIcon() . "'></i>" . __s("Uninstall", 'uninstall')];
    }
...
function plugin_uninstall_addDefaultWhere($itemtype)
{
    ...
    } elseif ($itemtype === 'PluginUninstallModelcontainer' && isset($_GET['id'])) {
        return "`glpi_plugin_uninstall_modelcontainers`.`plugin_uninstall_models_id` = '" . $_GET['id'] . "'";
    } elseif ($itemtype === 'PluginUninstallModelcontainerfield' && isset($_GET['id'])) {
