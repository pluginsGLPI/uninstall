        ...
        if ($pluginFieldsContainer->getFromDB($this->fields['plugin_fields_containers_id'])) {
            ...
            if ($model->fields['types_id'] != $model::TYPE_MODEL_UNINSTALL) {
                if ($model->fields['action_plugin_fields_replace'] == $model::PLUGIN_FIELDS_ACTION_ADVANCED) {
                    $dropdown = [
                        'values' => self::getActions($model::TYPE_MODEL_REPLACEMENT),
                        'value' => ($this->fields["action_replace"] ?? $this::ACTION_NONE),
                        'width' => '100%',
                    ];
                }
            }
            ...
            if ($model->fields['types_id'] != $model::TYPE_MODEL_REPLACEMENT) {
                if ($model->fields['action_plugin_fields_uninstall'] == $model::PLUGIN_FIELDS_ACTION_ADVANCED) {
                    $dropdown = [
                        'values' => self::getActions($model::TYPE_MODEL_UNINSTALL),
                        'value' => ($this->fields["action_uninstall"] ?? $this::ACTION_NONE),
                        'width' => '100%',
                    ];
                }
            }

            TemplateRenderer::getInstance()->display('@uninstall/modelcontainer_showForm.html.twig',[
                'modelValues'=> $model->fields,
                'pluginFieldsContainer' => [
                    'typeName' => $pluginFieldsContainer->getTypeName(),
                    'formUrlWithId' => $pluginFieldsContainer->getFormURLWithID($pluginFieldsContainer->getID()),
                    'values' => $pluginFieldsContainer->fields,
                ],
                'itemObjet' => $obj,
                'collspan' => ['title' => $titleColspan, 'action' => $actionColspan],
                'rand' => mt_rand(),
                'dropdown' => $dropdown,
            ]);
    ...
    public static function showListsForType($modelId, $type)
    {
        ...
        if (count($uninstallContainers)) {
            ...
            foreach ($uninstallContainers as $uninstallContainer) {
                $uninstallContainers[$uninstallContainer['id']]['hasfieldcontainer'] = false;
                if ($fieldContainer->getFromDB($uninstallContainer['plugin_fields_containers_id'])) {
                    $uninstallContainers[$uninstallContainer['id']]['hasFieldContainer'] = true;
                    $uninstallContainers[$uninstallContainer['id']]['link'] = PluginUninstallModelContainer::getFormURLWithID($uninstallContainer['id']);
                    $uninstallContainers[$uninstallContainer['id']]['fieldContainerValues'] = $fieldContainer->fields;
                    $uninstallContainers[$uninstallContainer['id']]['action'] = self::getActions()[$uninstallContainer[$actionProperty]];
                    ...
                    $uninstallContainers[$uninstallContainer['id']]['obj'] = $obj;
                }
            }

            TemplateRenderer::getInstance()->display('@uninstall/modelcontainer_showList.html.twig',[
                'uninstallContainers' => $uninstallContainers
            ]);
        } else {
